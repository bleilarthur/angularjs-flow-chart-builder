        ...
        dragstart: function (node) {
          return function (event) {
            ...
            if (dragAnimation == flowchartConstants.dragAnimationShadow) {
              var shadowElement = angular.element('<div style="position: absolute; opacity: 0.7; top: ' + getYCoordinate(dragOffset.y + event.clientY) + 'px; left: ' + getXCoordinate(dragOffset.x + event.clientX) + 'px; "><div><p style="padding: 0 15px;">' + nodeDraggingScope.draggedNode.name + '</p> </div></div>');
...
(function () {
  ...
  function canvasController($scope, Mouseoverfactory, Nodedraggingfactory, Modelfactory, Edgedraggingfactory, Edgedrawingservice, FlowchartCanvasService) {
    ...
    $scope.editNode = $scope.userCallbacks.editNode || angular.noop;
...
(function (module) {
  ...
  module.run(['$templateCache', function ($templateCache) {
    $templateCache.put('flowchart/node.html',
      ...
      '  <div class="inner-node {{ node.type }}">\n' +
      ...
      '    &times;\n' +
      '  </div>\n' +
      '  <div class="fc-nodeedit" ng-click="callbacks.editNode(node)">\n' +
      '    &#9998;\n' +
      '  </div>\n' +
      '</div>\n' +
      '');
